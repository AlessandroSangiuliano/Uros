...
#   - `shutdown` / `halt` from the ush prompt cleanly powers QEMU off
#     via ACPI.  `reboot` resets the CPU but QEMU's `-kernel` boot
#     mode loads the kernel image only once: after the reset the BIOS
#     has no bootable medium (disk0a is /mach_servers/, not a
#     boot-sector disk), so the VM stays dark.  Use `shutdown` and
#     relaunch this script to reboot, until a bootable disk image
#     lands (tracked in a follow-up issue).
