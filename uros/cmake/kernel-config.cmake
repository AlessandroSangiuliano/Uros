...
")
  ...
  #
  # #457 named the six operations it actually needs from the machine in
  # <device/device_machdep.h> and gave each architecture its own answers, and
  # built the config-space half: ECAM where the MCFG describes one and the
  # port pair where it does not -- both, because both are boards this tree
  # boots.
  #
  # ⚠️ STILL OFF here, and for one named reason rather than a general one.
  # ds_master_device_intr_register() installs a handler with take_irq(irq,
  # unit, SPL6, ...), which is the i386 interrupt-vector interface: a priority
  # level this machine's spl bridge does not have (#454) and a vector
  # allocation that here means routing an I/O APIC pin.  That is the last of
  # the six and it is design rather than adaptation, so it is not written in
  # the same breath as the ones that were.
  #
  # ⚠️ Still off for anything that is neither: with the knob off the subsystem
  # is ABSENT rather than faked -- it is not in ipc_kobject.c's mig_e[] table,
  # so a client that calls it gets MIG_BAD_ID.  That is the accurate answer,
  # where fourteen entry points returning a plausible code would be a lie a
  # driver could act on.
  #
  # ⚠️ And the DMA half is on here without an IOMMU behind it.  device_dma_*
  # hands a driver a physical address, which is what #432 exists to stop
  # being true; what #457 turned on is the config-space, MMIO and interrupt
  # half, and the isolation claim is not met until #432.
  if(NOT arch STREQUAL "i386")
