...
")
  ...
  #
  # It was off on x86-64 because device/device_master.c was i386 code that
  # happened to live in device/: it included <i386/ipl.h>, <i386/pio.h>,
  # <i386/ioapic.h> and <i386/misc_protos.h> outside any conditional, so
  # compiling it here pulled <mach/i386/vm_types.h> in behind them, where
  # vm_offset_t is thirty-two bits.
  #
  # #457 named the eight operations it actually needs from the machine in
  # <device/device_machdep.h> and gave each architecture its own answers.  Two
  # of them changed mechanism rather than spelling, and both changed it per
  # BOARD or per machine rather than per architecture:
  #
  #   config space   ECAM where the MCFG describes one, the port pair where it
  #                  does not -- both, because both are boards this tree boots.
  #
  #   claiming a     i386 writes a handler, a unit and a priority into three
  #   line           parallel arrays its dispatch reads.  Here it is a vector
  #                  (0x40 + irq, whose top four bits ARE its priority), a
  #                  claim in the IDT dispatch table, and an I/O APIC pin
  #                  pointed at it.  Which is why the operation takes no
  #                  priority argument: there is nowhere to put one that the
  #                  choice of vector has not already decided.
  #
  # ⚠️ ON HERE NOW, and the interrupt half is the part that had to be designed
  # rather than adapted.  It also found the one behaviour this machine has that
  # i386 does not: a device interrupt can be DEFERRED by the priority level,
  # and a deferred interrupt has already been acknowledged -- so its handler
  # must not acknowledge it again when the level drops.  <cpu/spl.h> had said a
  # handler could not be made deferrable without being read; this is the first
  # one that could be, and reading it is what that note was for.
  #
  # ⚠️ Sixteen lines, and the ISA ones.  A PCI device on a Q35 arrives on a
  # global system interrupt of 16 or above and reaching it needs the routing
  # table in the firmware's AML, which nothing here interprets yet.
  #
  # ⚠️ And the DMA half is on without an IOMMU behind it.  device_dma_* hands a
  # driver a physical address, which is what #432 exists to stop being true;
  # what #457 turned on is the config-space, MMIO and interrupt half, and the
  # isolation claim is not met until #432.
