...
/*
 *	#338: split page-table locks.  Each page-table page is locked by a bit
 *	in pt_lock_table, indexed by that PT page's own physical page number
 *	(pa_index) -- exactly as pv_lock_table indexes data pages.  pmap_enter
 *	and pmap_remove_range lock only the PT page they touch instead of the
 *	whole pmap->lock, so faults on different PT pages of one address space
 *	run in parallel.  Same size as pv_lock_table (one bit per managed page);
 *	a PT page is never simultaneously a pv-tracked data page, so the two
 *	bit-arrays never alias for the same pai with conflicting meaning.
 */
char *pt_lock_table; /* #338: per-PT-page lock bits */
...
/* #338: per-PT-page lock, keyed by the PT page's own pa_index. */
#define lock_pt_pai(pai) bit_lock(pai, (void *)pt_lock_table)
#define try_lock_pt_pai(pai) bit_lock_try(pai, (void *)pt_lock_table)
#define unlock_pt_pai(pai) bit_unlock(pai, (void *)pt_lock_table)

/*
 * #338: order a page-table store before the cpus_using load inside
 * PMAP_UPDATE_TLBS.  On x86 the only architectural reordering is store->load, so
 * a full fence is required so an activating CPU (set_dirbase: PMAP_CPU_SET then
 * the serializing set_cr3) is either flushed by us or already sees our new PTE;
 * the per-pmap lock used to provide this implicitly.  Built for i686 (no SSE2
 * mfence) -> a locked stack no-op, a full barrier on every x86.  UP needs none.
 */
#if NCPUS > 1
#define PMAP_TLB_WRITE_BARRIER() \
...
/*
 * #338: resident_count/wired_count are per-pmap but are now bumped under the
 * per-PT-page pt-lock, so concurrent pmap_enter/remove/page_protect on
 * different PT pages of one pmap would race a plain ++/--.  Decision 3: atomic
 * add for v1 (per-CPU counters are the scaling ceiling but cost ~KB/pmap --
 * revisit if the stats cache line shows up on bare metal).  integer_t is int,
 * which is long-width on i386, so the cast to long* is representation-safe.
 */
#if NCPUS > 1
#define PMAP_STAT_ADD(field, delta) \
	((void)atomic_add_fetchl((long *)&(field), (long)(delta)))
#else
#define PMAP_STAT_ADD(field, delta) ((field) += (delta))
#endif
...
struct pmap_update_list {
	decl_simple_lock_data(, lock) int count;
	...
	vm_prot_t prot)
{
	...
	template = pa_to_pte(start) | INTEL_PTE_NCACHE | INTEL_PTE_REF | INTEL_PTE_MOD | INTEL_PTE_WIRED | INTEL_PTE_VALID;
	...
	vm_offset_t load_start)
{
	...
	template = pa_to_pte(avail_start) | INTEL_PTE_VALID | INTEL_PTE_WRITE | INTEL_PTE_GLOBAL;
	...
	for (va = virtual_avail; va < virtual_end; va += INTEL_PGBYTES) {
		if (pte >= ptend) {
			...
			*pde = PA_TO_PTE((vm_offset_t)pte) | INTEL_PTE_VALID | INTEL_PTE_WRITE | INTEL_PTE_GLOBAL;
			pde++;
		}
		WRITE_PTE_FAST(pte, template)
		pte++;
	 ...
	 */
	{
		...
		if (VM_MAX_KERNEL_LOADED_ADDRESS > virtual_end) {
			vm_size_t loaded_need =
				trunc_page(VM_MAX_KERNEL_LOADED_ADDRESS) - virtual_end;
	...
	*(int *)&template = 0;
	 ...
	 */
	kpde[PTDPTDI] = pa_to_pte((vm_offset_t)kpde - VM_MIN_KERNEL_ADDRESS) | INTEL_PTE_VALID | INTEL_PTE_WRITE;
...
void pmap_init(void)
{
	...
	s = (vm_size_t)(sizeof(struct pv_entry) * npages + pv_lock_table_size(npages) /* pv_lock_table */
			+ pv_lock_table_size(npages) /* #338: pt_lock_table */
			+ npages);
	...
	pt_lock_table = (char *)addr; /* #338 */
	addr = (vm_offset_t)(pt_lock_table + pv_lock_table_size(npages));
	...
	vm_size_t size)
{
	...
	while ((p = pmap_cache_list) == PMAP_NULL) {
		...
		if (kmem_alloc_wired(kernel_map, &dirbases,
			    pmap_alloc_chunk * INTEL_PGBYTES) != KERN_SUCCESS)
			panic("pmap_create.1");
		...
		for (i = pmap_alloc_chunk; i > 0; i--) {
			 ...
			 */
			{
				...
				p->dirbase[PTDPTDI] = pa_to_pte(p->pdirbase) | INTEL_PTE_VALID | INTEL_PTE_WRITE;
			}
 ...
 *
 *	The caller holds the write lock for the PT page (the #338 pt-lock, or
 *	pmap->lock for kernel_pmap / UP) and passes its token in `ptl`
 *	(pa_index of the PT page, or -1 for the per-pmap lock).
...
/*
 * #338: drop / reacquire just the lock a PT-page writer holds, for
 * pmap_remove_range's PVH back-off (which keeps spl and the system lock).
 * ptl >= 0 is a user PT-page pt-lock; ptl < 0 is the per-pmap lock.  Only ever
 * reached on SMP (on UP LOCK_PVH_TRY never fails, so the back-off is dead).
 */
static void
pmap_ptl_unlock(register pmap_t pmap, int ptl)
{
#if NCPUS > 1
	if (ptl >= 0) {
		unlock_pt_pai(ptl);
...
static void
pmap_ptl_lock(register pmap_t pmap, int ptl)
{
#if NCPUS > 1
	if (ptl >= 0) {
		lock_pt_pai(ptl);
...
void pmap_remove_range(
	...
	pt_entry_t *spte,
	pt_entry_t *epte,
	int ptl) /* #338: caller's lock token */
{
	...
	for (cpte = spte; cpte < epte;
		 ...
		 */
		if (!LOCK_PVH_TRY(pai)) {
			pmap_ptl_unlock(pmap, ptl); /* #338: drop pt-lock or pmap->lock */
			LOCK_PVH(pai);
			UNLOCK_PVH(pai);
			pmap_ptl_lock(pmap, ptl);
	...
	PMAP_STAT_ADD(pmap->stats.resident_count, -num_removed);
	assert(pmap->stats.wired_count >= num_unwired);
	PMAP_STAT_ADD(pmap->stats.wired_count, -num_unwired);
}

/*
 * #338: pmap_remove for a user pmap on SMP -- lock each PT page individually
 * (pt-lock + pmd_same recheck) and remove its slice of [s, e), so removals on
 * different PT pages of one address space do not serialize on one lock.  The TLB
 * flush is per-PT-page, ordered after the PTE clears.  The range need not be
 * atomic across PT pages: the VM map lock already serializes operations on a VA
 * range, and each PTE is protected by its own PT page's lock.
 */
#if NCPUS > 1
static void
pmap_remove_user(register pmap_t map, vm_offset_t s, vm_offset_t e)
{
	spl_t spl;
	...
	while (s < e) {
		...
		if ((pde & INTEL_PTE_VALID) == 0) {
			SPLX(spl);
			s = l;
			continue; /* no PT page maps this 4 MB */
		}
		pai = pa_index(pte_to_pa(pde));
		lock_pt_pai(pai);
		if (*pdp != pde) { /* pmd_same: collect/expand raced */
			unlock_pt_pai(pai);
			SPLX(spl);
			continue; /* retry the same s */
		}
		spte = &((pt_entry_t *)ptetokv(pde))[ptenum(s)];
		epte = &spte[intel_btop(l - s)];
		pmap_remove_range(map, s, spte, epte, pai);
		PMAP_TLB_WRITE_BARRIER(); /* store before cpus_using load */
		PMAP_UPDATE_TLBS(map, s, l);
		unlock_pt_pai(pai);
		SPLX(spl);
		s = l;
	}
	...
	vm_offset_t e)
{
...
#if NCPUS > 1
	if (map != kernel_pmap) {
		pmap_remove_user(map, s, e); /* #338: per-PT-page locking */
		return;
	}
#endif
	...
	while (s < e) {
		...
		if (*pde & INTEL_PTE_VALID) {
			...
			epte = &spte[intel_btop(l - s)];
			pmap_remove_range(map, s, spte, epte, -1); /* #338 */
		}
		s = l;
		pde++;
	}
...
/*
 * #338: lock the PT page that maps `va` in `pmap` for a protocol-3 pv-list walk
 * (pmap_page_protect, phys_attribute_set/clear).  The caller holds the data
 * page's PVH, which pins the PT page -- it cannot be collected without removing
 * this pv entry, and that needs the PVH -- so the PDE is stable and no pmd_same
 * recheck is needed.  Returns the lock token (pt-lock pai for a user pmap, -1
 * for kernel_pmap / UP); release with pmap_ptl_unlock().
 */
static int
pmap_pv_lock(register pmap_t pmap, vm_offset_t va)
{
#if NCPUS > 1
	if (pmap != kernel_pmap) {
		register int pai = pa_index(pte_to_pa(*pmap_pde(pmap, va)));
		lock_pt_pai(pai);
		return pai;
	}
#endif
	...
	vm_prot_t prot)
{
	...
	if (pv_h->pmap != PMAP_NULL) {
		...
		prev = pv_e = pv_h;
		do {
			register vm_offset_t va = pv_e->va;
			int ptl;

			pmap = pv_e->pmap;
			/*
			 * #338: lock the PT page that maps va (pt-lock for a user pmap,
			 * pmap->lock for kernel_pmap) instead of the whole pmap.  The PVH
			 * we hold pins the PT page; protocol-2's LOCK_PVH_TRY back-off
			 * breaks the PVH<->pt-lock ordering, so no recheck is needed.
			 */
			ptl = pmap_pv_lock(pmap, va);

			pte = pmap_pte(pmap, va);
			/* assert(*pte & INTEL_PTE_VALID); assert(pte_to_phys(*pte)==phys); */
			...
			if (remove || pmap == kernel_pmap) {
				...
				PMAP_STAT_ADD(pmap->stats.resident_count, -1);
			...
			/*
			 * #338: flush AFTER the PTE store (was before the modify),
			 * ordered by the barrier so an activating CPU is flushed or sees
			 * the new PTE.
			 */
			PMAP_TLB_WRITE_BARRIER();
			PMAP_UPDATE_TLBS(pmap, va, va + PAGE_SIZE);

			pmap_ptl_unlock(pmap, ptl);
...
/*
 * #338: acquire the write lock for user VA `v` in `pmap` and return the locked
 * PT page's pai, with SPLVM held and the PT page present.  On UP, or for
 * kernel_pmap (not split -- its PT pages are not VM-managed and have no
 * pa_index), this takes the per-pmap lock via PMAP_READ_LOCK and returns -1.
 * For a user pmap on SMP the PDE is read locklessly; after taking the PT page's
 * pt-lock we recheck the PDE still points at the same PT page (pmd_same) and
 * retry if a concurrent pmap_collect cleared it or pmap_expand replaced it.  The
 * #330 type-stable PT pool makes locking a possibly-recycled PT page safe.
 */
static int
pmap_enter_lock(register pmap_t pmap, vm_offset_t v, spl_t *splp)
{
#if NCPUS > 1
	if (pmap != kernel_pmap) {
		register pt_entry_t *pdp = pmap_pde(pmap, v);
		register pt_entry_t pde;
		register int pai;
		...
		for (;;) {
			...
			lock_pt_pai(pai);
			if (*pdp == pde)
				return pai; /* PDE stable: PT page is ours */
			unlock_pt_pai(pai); /* collect/expand moved it */
			SPLX(*splp);
		}
	}
#endif /* NCPUS > 1 */
	PMAP_READ_LOCK(pmap, *splp);
	return -1;
}

/*
 * #338: release what pmap_enter_lock() took -- a user PT-page lock (ptl >= 0) or
 * the per-pmap lock (ptl < 0: kernel_pmap or UP).
 */
static void
pmap_enter_unlock(register pmap_t pmap, int ptl, spl_t spl)
{
#if NCPUS > 1
	if (ptl >= 0) {
		unlock_pt_pai(ptl);
	...
	boolean_t wired)
{
	...
	spl_t spl;
	vm_offset_t old_pa;
	int ptl; /* #338: pmap_enter_lock token */
	 ...
	 */
	for (;;) {
		ptl = pmap_enter_lock(pmap, v, &spl); /* #338: pt-lock (user/SMP) or pmap->lock */

		/*
		 *	Expand pmap to include this pte.  For a user pmap on SMP
		 *	pmap_enter_lock() already expanded and pt-locked the PT page, so
		 *	pmap_pte is non-NULL and this loop does not iterate; it remains for
		 *	kernel_pmap / UP (where pmap_expand panics for the kernel, as before).
		 */
		...
		while ((pte = pmap_pte(pmap, v)) == PT_ENTRY_NULL) {
			...
			ptl = pmap_enter_lock(pmap, v, &spl);
		}
		/*
		...
		if (old_pa == pa) {
			...
			if (wired) {
				...
				if (!iswired(*pte))
					PMAP_STAT_ADD(pmap->stats.wired_count, 1);
			} else {
				if (iswired(*pte)) {
			...
			/* #338: flush AFTER the store (was before) so a concurrently
			 * activating CPU is either flushed or sees the new PTE. */
			PMAP_TLB_WRITE_BARRIER();
			PMAP_UPDATE_TLBS(pmap, v, v + PAGE_SIZE);
		...
		if (old_pa != (vm_offset_t)0) {
			...
			/*
			 * #338: the TLB flush for this va now happens once at the end, after
			 * the new PTE is written (PMAP_UPDATE_TLBS below), so the store is
			 * ordered before the cpus_using load.  The old PTE is cleared under
			 * the pt-lock here; a concurrent faulter blocks on that lock.
			 */
		...
		/* #338: flush AFTER writing the new PTE (and any old-PTE clear above),
		 * with the store ordered before the cpus_using load (was: flush first). */
		PMAP_TLB_WRITE_BARRIER();
		PMAP_UPDATE_TLBS(pmap, v, v + PAGE_SIZE);
	...
	boolean_t wired)
{
	...
	/*
	 * #338: lock the PT page that maps v (pt-lock for a user pmap, pmap->lock
	 * for kernel_pmap / UP) -- the WIRED bit must change under the same lock as
	 * pmap_collect's wired-mapping check and protocol-2's PTE writes.  No TLB
	 * flush: WIRED is a software bit the hardware ignores.
	 */
	ptl = pmap_enter_lock(map, v, &spl);
	...
	if (wired && !iswired(*pte)) {
		 ...
		 *	wiring down mapping
		 */
		PMAP_STAT_ADD(map->stats.wired_count, 1);
		i = ptes_per_vm_page;
		do {
			*pte++ |= INTEL_PTE_WIRED;
		} while (--i > 0);
	...
	register vm_offset_t v)
{
	...
	do {
		*pdp = pa_to_pte(pa) | INTEL_PTE_VALID | INTEL_PTE_USER | INTEL_PTE_WRITE;
		pdp++;
		pa += INTEL_PGBYTES;
	} while (--i > 0);
	PMAP_READ_UNLOCK(map, spl);
	...
	pmap_t p)
{
	 ...
	 */
	/*
	 * #338: PMAP_READ_LOCK is now the dir-lock -- it excludes pmap_expand
	 * (the other PDE writer).  The old whole-range PMAP_UPDATE_TLBS here was
	 * "flush then clear", unsafe once the pmap lock no longer brackets a
	 * concurrent activate; each removed PT page is flushed below after its
	 * PTEs and PDE are cleared (barrier-ordered).
	 */
		...
		if (*pdp & INTEL_PTE_VALID)
			if (*pdp & INTEL_PTE_REF) {
				*pdp &= ~INTEL_PTE_REF;
				...
				va = pdetova(pdp - p->dirbase);

				/*
				 * #338: take this PT page's pt-lock (under the dir-lock we hold)
				 * before inspecting or clearing its PTEs, so a concurrent
				 * pmap_enter / protocol-3 writer holding the pt-lock is excluded.
				 * Order is dir-lock -> pt-lock (canonical).
				 */
				pai = pa_index(pa);
				lock_pt_pai(pai);
				...
				if (!wired) {
					 ...
					 */
					pmap_remove_range(p, va, ptp, eptp, pai);
					...
					/*
					 * #338: flush this PT page's range after clearing its PTEs and
					 * PDE, barrier-ordered before the cpus_using load.
					 */
					PMAP_TLB_WRITE_BARRIER();
					PMAP_UPDATE_TLBS(p, va, va + PDE_MAPPED_SIZE);
					...
					unlock_pt_pai(pai); /* #338: wired -- keep it */
				}
			}
	}
	PMAP_READ_UNLOCK(p, spl);
	...
	int bits)
{
	...
	if (pv_h->pmap != PMAP_NULL) {
		...
		for (pv_e = pv_h; pv_e != PV_ENTRY_NULL; pv_e = pv_e->next) {
			...
			pmap = pv_e->pmap;
			/* #338: pt-lock of the PT page mapping va (kernel: pmap->lock). */
			ptl = pmap_pv_lock(pmap, va);
			pte = pmap_pte(pmap, va);
			...
			/*
			 * #338: flush AFTER the store.  #304: a reference-only clear is
			 * advisory -> local flush (a stale accessed bit just re-faults;
			 * pmap_clear_reference runs on every scanned page, so a cross-CPU
			 * shootdown here was an IPI storm).  A MODIFY clear does the
			 * cross-CPU shootdown, barrier-ordered before the cpus_using load
			 * so an activating CPU is flushed or sees the cleared dirty bit.
			 */
 ...
 *	follow-up.
 */
void pmap_tlb_shootdown_handler(void)
{
	register int my_cpu;
...
void db_show_page(vm_offset_t pa)
{
	pv_entry_t pv_h;
	int pai;
	char attr;
