	...
	mach_msg_type_number_t	*count)
{
	...
	case HOST_IPC_CACHE_INFO:
		{ register host_ipc_cache_info_t	cache_info;
		  extern void ikm_cache_stats(unsigned int *, unsigned int *);
		  unsigned int kmsg_tries, kmsg_misses;

		if (*count < HOST_IPC_CACHE_INFO_COUNT)
			return(KERN_FAILURE);

		ikm_cache_stats(&kmsg_tries, &kmsg_misses);	/* #323: per-CPU fold */
		cache_info = (host_ipc_cache_info_t) info;
		cache_info->tries      = kmsg_tries;
		cache_info->misses     = kmsg_misses;
