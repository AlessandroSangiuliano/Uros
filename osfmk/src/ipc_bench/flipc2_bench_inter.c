...
flipc2_child_batch_echo_entry(void)
{
    ...
    for (;;) {
        ...
        FLIPC2_READ_FENCE();
        uint32_t avail = *fwd_prod_tail - head;
        uint32_t tail = *rev_prod_tail;
        uint32_t j;
                   ...
                   mach_port_t *out_task, mach_port_t *out_thread)
{
    ...
    /* Volatile field offsets: compute from handle pointers */
...
/* ===================================================================
 * Isolated channel inter-task RPC benchmark.
 *
 * Same as bench_flipc2_inter_rpc but creates channels with
 * FLIPC2_CREATE_ISOLATED and shares them with per-role protections
 * via flipc2_channel_share_isolated.
 *
 * Parent: producer on fwd, consumer on rev.
 * Child:  consumer on fwd, producer on rev.
 * =================================================================== */

void
bench_flipc2_isolated_inter_rpc(const char *label, int data_size, int iters)
{
    flipc2_channel_t    fwd_ch, rev_ch;
    mach_port_t         fwd_sem, rev_sem;
    mach_port_t         child_task, child_thread;
    ...
    if (chan_size < FLIPC2_CHANNEL_SIZE_MIN_ISOLATED)
        chan_size = FLIPC2_CHANNEL_SIZE_MIN_ISOLATED;

    ret = flipc2_channel_create_ex(chan_size, FLIPC2_BENCH_RING_ENTRIES,
                                   FLIPC2_CREATE_ISOLATED, &fwd_ch, &fwd_sem);
    if (ret != FLIPC2_SUCCESS) {
        printf("  %s: fwd create failed %d\n", label, ret);
        return;
    }
    ...
    ret = flipc2_channel_create_ex(chan_size, FLIPC2_BENCH_RING_ENTRIES,
                                   FLIPC2_CREATE_ISOLATED, &rev_ch, &rev_sem);
    if (ret != FLIPC2_SUCCESS) {
        printf("  %s: rev create failed %d\n", label, ret);
        flipc2_channel_destroy(fwd_ch);
        return;
     ...
     * fwd: child is consumer, rev: child is producer */
    ret = flipc2_channel_share_isolated(fwd_ch, child_task,
                                         FLIPC2_ROLE_CONSUMER,
                                         &child_fwd_addr);
    if (ret != FLIPC2_SUCCESS) {
        printf("  %s: share_isolated fwd failed %d\n", label, ret);
        task_terminate(child_task);
        flipc2_channel_destroy(rev_ch);
        flipc2_channel_destroy(fwd_ch);
        return;
    ...
    ret = flipc2_channel_share_isolated(rev_ch, child_task,
                                         FLIPC2_ROLE_PRODUCER,
                                         &child_rev_addr);
    if (ret != FLIPC2_SUCCESS) {
        printf("  %s: share_isolated rev failed %d\n", label, ret);
        task_terminate(child_task);
        flipc2_channel_destroy(rev_ch);
        flipc2_channel_destroy(fwd_ch);
        return;
    ...
    /* Share semaphore ports with child */
    ret = flipc2_semaphore_share(fwd_sem, child_task, FLIPC2_CHILD_SEM_FWD);
    if (ret != FLIPC2_SUCCESS) {
        printf("  %s: semaphore_share fwd failed %d\n", label, ret);
        task_terminate(child_task);
